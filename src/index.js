  ...
  getRequestHandler(app, customHandler) {
    ...
    return (req, res) => {
      ...
      if (route) {
        ...
        const staticRegexp = /\[.*?\]/gm;
        [...Array.from(page.matchAll(staticRegexp))].forEach((match) => {
          const matchedRegex = match[0];
          const matchedRegexKey = match[0].replace("[", "").replace("]", "");
          if (page) {
            page = page.replace(matchedRegex, query[matchedRegexKey]);
            isStaticPage = true;
          }
        });
        if (customHandler) {
          ...
          if (isStaticPage) {
            req.url = page;
            nextHandler(req, res, page);
          } else app.render(req, res, route.page, query);
        }
      } else {
